<script lang="ts">
	...
	import { getGameHref, topCategories } from '$lib/data';
...
>
	...
	<div class="mt-auto flex gap-2 border-t border-console-border pt-4">
		{#if game.nightly}
			<span class="flex-1 rounded-sm border border-console-green/30 bg-console-green/10 px-3 py-2 text-center font-mono text-[0.65rem] uppercase tracking-[0.12em] text-console-green">
				↓ Nightly
			</span>
		{/if}
		<span class="flex-1 rounded-sm border border-console-border px-3 py-2 text-center font-mono text-[0.65rem] uppercase tracking-[0.12em] text-console-muted group-hover:border-console-green group-hover:text-console-green">
			Mionsonraí
		</span>
	</div>
</a>
